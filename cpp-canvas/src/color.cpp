...
namespace canvas {
    ...
    Color Color::parseHex(const std::string& hex) {
        ...
        case 7: // #rrggbb
            rgba.r =
                std::strtol(hex.substr(1, 2).c_str(), nullptr, 16) / 255.0f;
            rgba.g =
                std::strtol(hex.substr(3, 2).c_str(), nullptr, 16) / 255.0f;
            rgba.b =
                std::strtol(hex.substr(5, 2).c_str(), nullptr, 16) / 255.0f;
            break;
        case 9: // #rrggbbaa
            rgba.r =
                std::strtol(hex.substr(1, 2).c_str(), nullptr, 16) / 255.0f;
            ...
            rgba.b =
                std::strtol(hex.substr(5, 2).c_str(), nullptr, 16) / 255.0f;
            rgba.a =
                std::strtol(hex.substr(7, 2).c_str(), nullptr, 16) / 255.0f;
            break;
        }
        return Color(rgba);
    }
